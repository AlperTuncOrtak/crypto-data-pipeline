      ...
      {visible && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-2xl bg-[var(--bg-base)] border border-[var(--border-base)] rounded-3xl shadow-2xl flex flex-col"
          >
            <div className="p-6 border-b border-[var(--border-base)]">
              <h2 className="text-xl font-semibold text-[var(--text-main)] tracking-tight">Terms of Service & Risk Disclosure</h2>
              <p className="text-sm text-[var(--text-muted)] mt-1">Please read and acknowledge the following terms before proceeding.</p>
            </div>

            <div className="p-6">
              <div className="bg-[var(--bg-elevated)] border border-[var(--border-subtle)] rounded-2xl p-4 h-64 overflow-y-auto custom-scrollbar">
                
                {regionWarning && (
                  <div className="mb-6 pb-6 border-b border-[var(--border-subtle)]">
                    <h3 className="text-sm font-bold text-[var(--text-main)] mb-2 uppercase tracking-wider">{regionWarning.title}</h3>
                    <p className="text-[13px] text-[var(--text-muted)] leading-relaxed">{regionWarning.text}</p>
                  </div>
                )}

                <div className="space-y-6">
                  <section>
                    <h3 className="text-sm font-bold text-[var(--text-main)] mb-2 uppercase tracking-wider">1. Not Financial Advice</h3>
                  ...
                  <section>
                    <h3 className="text-sm font-bold text-[var(--text-main)] mb-2 uppercase tracking-wider">2. Assumption of Risk</h3>
                    <p className="text-[13px] text-[var(--text-muted)] leading-relaxed">
                  ...
                  <section>
                    <h3 className="text-sm font-bold text-[var(--text-main)] mb-2 uppercase tracking-wider">3. No Guarantees</h3>
                    <p className="text-[13px] text-[var(--text-muted)] leading-relaxed">
                  ...
                  <section>
                    <h3 className="text-sm font-bold text-[var(--text-main)] mb-2 uppercase tracking-wider">4. Limitation of Liability</h3>
                    <p className="text-[13px] text-[var(--text-muted)] leading-relaxed">
              ...
              >
                <div className={`mt-0.5 flex-shrink-0 w-5 h-5 rounded border flex items-center justify-center transition-all duration-150 ${checked ? 'bg-white border-white' : 'bg-[var(--bg-elevated)] border-gray-600 group-hover:border-gray-400'}`}>
                  <Check size={14} className={`text-black transition-opacity duration-150 ${checked ? 'opacity-100' : 'opacity-0'}`} strokeWidth={3} />
                </div>
                <span className="text-sm text-gray-300 select-none group-hover:text-[var(--text-main)] transition-colors">
                  I have read and agree to the Terms of Service and Risk Disclosure.
                </span>
              </label>
            </div>

            <div className="p-4 border-t border-[var(--border-base)] bg-[var(--bg-base)] rounded-b-xl flex justify-end gap-3">
              <button
                onClick={handleDecline}
                className="px-5 py-2.5 rounded-2xl text-sm font-medium text-[var(--text-muted)] hover:text-[var(--text-main)] hover:bg-[var(--border-subtle)] transition-colors"
              >
                Decline
              </button>
              <button
