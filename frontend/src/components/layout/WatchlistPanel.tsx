...
export default function WatchlistPanel({
  ...
  return (
    <div className="flex flex-col h-full bg-[#0a0b0d]/50">
      <div className="px-6 py-5 border-b border-[#273951]/50 bg-[#16181c]/80 backdrop-blur-md">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-lg text-white tracking-tight">Watchlist</span>
            {watchlist.length > 0 && (
              <span className="text-xs px-2 py-0.5 rounded-full bg-white/5 border border-white/10 text-gray-300 font-mono font-medium">
                {watchlist.length}
              </span>
            )}
          </div>
          ...
          <div className="flex gap-2 relative">
            <motion.button
              ...
              className={`w-9 h-9 rounded-xl flex items-center justify-center border transition-all ${
                showSort ? "bg-white/10 border-white/20 text-white" : "bg-white/5 border-white/5 text-gray-400 hover:text-white"
              }`}
            >
              <ArrowUpDown size={14} />
            </motion.button>
            ...
            <AnimatePresence>
              {showSort && (
                <motion.div
                  ...
                  className="absolute top-12 right-12 w-40 bg-[#16181c]/95 backdrop-blur-xl border border-[#273951]/50 rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.8)] overflow-hidden z-50 p-2"
                >
                  {SORT_OPTIONS.map((o) => (
                    <button
                      key={o.id}
                      ...
                      className={`w-full text-left px-3 py-2 text-sm font-medium rounded-xl transition-all ${
                        sort === o.id ? "bg-[var(--accent)]/20 text-[var(--accent)]" : "text-gray-400 hover:bg-white/5 hover:text-white"
                      }`}
                    >
                      {o.label}
                    </button>
            ...
            <motion.button
              ...
              className={`w-9 h-9 rounded-xl flex items-center justify-center border transition-all ${
                ...
                showAdd ? "bg-[var(--accent)]/20 border-[var(--accent)]/40 text-[var(--accent)] shadow-[0_0_15px_rgba(83,58,253,0.3)]" : "bg-white/5 border-white/5 text-gray-400 hover:text-white"
              }`}
            >
              <Plus size={16} />
            </motion.button>
        ...
        <AnimatePresence>
          {showAdd && (
            ...
            >
              <div className="flex items-center gap-3 px-4 py-3 bg-[#0a0b0d]/50 border border-[var(--accent)]/30 rounded-xl shadow-[inset_0_0_10px_rgba(83,58,253,0.1)]">
              ...
              {searchResults.length > 0 && (
                <div className="absolute top-full left-0 right-0 mt-2 bg-[#16181c]/95 backdrop-blur-xl border border-[#273951]/50 rounded-2xl shadow-[0_20px_40px_rgba(0,0,0,0.8)] overflow-hidden z-50 p-2">
                  {searchResults.map((coin) => (
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-gray-500 truncate w-32">{coin.name}</div>
                      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3" style={{ scrollbarWidth: 'none' }}>
        {watchlist.length === 0 ? (
          <div className="flex flex-col items-center justify-center h-full opacity-50">
            ...
            <p className="text-sm text-gray-400 text-center max-w-[200px]">Use the + button above to start tracking your favorite coins.</p>
          </div>
        ) : (
          <AnimatePresence>
            {watchedCoins.map((coin, i) => {
              ...
              return (
                <motion.div
                  ...
                  key={coin.symbol}
                  className="group relative flex items-center gap-3 p-3 bg-[#16181c] border border-white/[0.05] rounded-2xl hover:bg-[#1a1c23] hover:border-[var(--accent)]/30 hover:shadow-[0_0_30px_rgba(83,58,253,0.1)] transition-all cursor-pointer"
                ...
                >
                  ...
                  <div className="relative">
                    ...
                    {coin.market_cap_rank && (
                      <div className="absolute -bottom-1 -right-1 text-[9px] font-bold bg-[#0a0b0d] border border-white/10 px-1 rounded text-gray-400">
                        {coin.market_cap_rank}
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-bold text-base tracking-tight text-white font-mono flex items-center gap-2">
                  ...
                  <button
                    ...
                    className="absolute -right-2 -top-2 w-7 h-7 bg-[#0a0b0d] border border-white/10 rounded-full flex items-center justify-center text-gray-500 opacity-0 group-hover:opacity-100 hover:text-red-400 hover:border-red-500/30 hover:bg-red-500/10 transition-all shadow-lg"
                  >
                    <Trash2 size={12} />
                  </button>
                </motion.div>
          ...
          >
            ...
            <div className="relative z-10">
              <div className="flex items-center justify-between mb-2">
                ...
                <button onClick={() => setShowLimitAlert(false)} className="text-gray-500 hover:text-white">
                  <X size={14} />
                </button>
              </div>
              <p className="text-xs text-gray-400 mb-3">Free plan includes up to {limit} coins. Upgrade to Pro for unlimited tracking.</p>
              <button
                onClick={() => navigate("/pricing")}
                className="w-full py-2 bg-[var(--accent)] text-white font-bold text-xs rounded-xl hover:scale-[1.02] active:scale-[0.98] transition-all"
              >
                Upgrade to Pro
              </button>
            </div>
      ...
      {watchlist.length > 0 && (
        <div className="px-6 py-4 border-t border-[#273951]/50 bg-[#16181c]/80 backdrop-blur-md flex items-center justify-between">
          <div className="text-xs font-medium text-gray-500">
            <span className="text-white font-mono">{watchlist.length}</span> {limit ? `/ ${limit}` : ""} Tracked
          </div>
          <button
