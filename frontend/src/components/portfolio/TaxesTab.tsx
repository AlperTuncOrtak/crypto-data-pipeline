  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between pb-4 border-b border-[var(--border-subtle)]">
        <div>
          <h2 className="text-xl font-bold text-[var(--text-main)] mb-1">Tax Center</h2>
          <p className="text-[13px] text-[var(--text-muted)]">Estimated capital gains and losses based on FIFO method</p>
        </div>
        <button
          onClick={handleExport}
          className="mt-4 md:mt-0 flex items-center gap-2 bg-gradient-to-r from-blue-600/20 to-blue-800/20 hover:from-blue-600/40 hover:to-blue-800/40 text-blue-400 border border-blue-500/30 font-bold py-2.5 px-5 rounded-3xl text-[13px] transition-all shadow-[0_0_20px_var(--accent)] hover:shadow-[0_0_30px_rgba(59,130,246,0.3)]"
        >
          <Download size={16} />
          Export CSV Report
        </button>
      ...
      <div className="p-6 rounded-[20px] bg-gradient-to-br from-orange-500/10 to-transparent border border-orange-500/20 flex flex-col md:flex-row items-start md:items-center gap-6 mt-6">
        ...
        <div className="flex-1">
          <h4 className="text-lg font-black text-[var(--text-main)] mb-1">Estimated Tax Liability</h4>
      ...
      <div className="mt-8">
        <h3 className="text-lg font-bold text-[var(--text-main)] mb-4">Recent Realized Events</h3>
          ...
          {taxData.allRealized.length > 0 ? (
            <div className="overflow-x-auto">
                ...
                <tbody className="text-[13px] text-gray-300 divide-y divide-white/5">
                  {taxData.allRealized.slice(0, 10).map((event: any, idx: number) => (
                    <tr key={idx} className="hover:bg-white/[0.02] transition-colors">
                      <td className="px-6 py-4 font-bold text-[var(--text-main)]">{event.symbol.toUpperCase()}</td>
             ...
             <div className="p-12 text-center flex flex-col items-center justify-center opacity-50">
              ...
              <h3 className="text-lg font-bold text-[var(--text-main)] mb-2">No Realized Events</h3>
