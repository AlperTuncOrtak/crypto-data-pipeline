  ...
  return (
    <section ref={containerRef} className="relative z-10 pt-32 pb-20 flex flex-col items-center justify-start min-h-[140vh] perspective-[2000px] overflow-hidden">
      ...
      <div className="flex flex-col items-center text-center max-w-[900px] mx-auto px-6 relative z-20">
        ...
        >
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-[var(--border-base)] bg-white/[0.02] hover:bg-white/[0.05] transition-colors">
            <span className="px-2 py-0.5 rounded-full bg-white/10 text-[var(--text-main)] text-[10px] font-bold uppercase tracking-wider">New</span>
        ...
        <motion.h1 
          ...
          className="text-6xl sm:text-7xl md:text-[5.5rem] font-black tracking-tighter leading-[1.05] mb-6 text-[var(--text-main)]"
        >
        ...
        <motion.p 
          ...
          className="text-lg md:text-xl text-[var(--text-muted)] font-medium max-w-2xl leading-relaxed mb-10"
        >
        ...
        >
          <button 
            ...
            className="w-full sm:w-auto px-6 py-3 rounded-2xl bg-white text-[#000000] font-semibold text-sm hover:bg-gray-200 transition-colors flex items-center justify-center gap-2"
          >
      ...
      <div className="w-full max-w-[1200px] px-6 mt-20 relative z-10 pointer-events-none">
        <motion.div
          ...
          className="relative w-full aspect-[16/10] sm:aspect-[16/9] rounded-3xl sm:rounded-[24px] border border-[var(--border-base)] bg-[var(--bg-base)] shadow-[0_0_80px_rgba(255,255,255,0.05)] overflow-hidden flex flex-col pointer-events-auto"
        >
          {/* Mac Header */}
          <div className="h-10 shrink-0 border-b border-[var(--border-subtle)] bg-[var(--bg-base)] flex items-center px-4 gap-2 z-20">
            <div className="flex gap-2">
              <div className="w-3 h-3 rounded-full bg-white/10 border border-[var(--border-base)]"></div>
              <div className="w-3 h-3 rounded-full bg-white/10 border border-[var(--border-base)]"></div>
              <div className="w-3 h-3 rounded-full bg-white/10 border border-[var(--border-base)]"></div>
            </div>
            <div className="flex-1 flex justify-center">
              <div className="px-3 py-1 rounded-2xl bg-white/[0.03] border border-[var(--border-subtle)] flex items-center gap-2">
                <Lock size={10} className="text-[var(--text-muted)]" />
                <span className="text-[10px] text-[var(--text-muted)] font-mono">cryptoneko.app/terminal</span>
              </div>
            </div>
          </div>
          
          {/* Main Terminal Layout */}
          <div className="flex-1 flex overflow-hidden text-[var(--text-main)] relative">
            
            {/* Sidebar */}
            <div className="w-16 sm:w-48 lg:w-56 border-r border-[var(--border-subtle)] bg-[var(--bg-base)]/50 flex flex-col pt-4 shrink-0 z-20">
              ...
              <div className="flex flex-col gap-1 px-2">
                {tabs.map(tab => (
                  <button 
                    ...
                    className={`flex items-center justify-center sm:justify-start gap-3 px-3 py-2 rounded-2xl transition-colors cursor-pointer ${activeTab === tab.id ? "bg-white/5 border border-[var(--border-subtle)] text-[var(--text-main)]" : "hover:bg-white/[0.02] text-[var(--text-muted)] border border-transparent"}`}
                  >
                    <tab.icon size={16} />
...
function MarketTab() {
  return (
    ...
    >
      {/* Header */}
      <div className="h-16 shrink-0 border-b border-[var(--border-subtle)] flex items-center justify-between px-4 sm:px-6">
        <div className="flex items-center gap-2 sm:gap-4 overflow-hidden">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-[8px] sm:text-[10px] font-mono text-[var(--text-muted)]">$64,230.50</span>
          </div>
          <div className="flex flex-col ml-2 sm:ml-4">
            <span className="text-[8px] sm:text-[10px] text-[var(--text-muted)] uppercase font-semibold">24h Change</span>
      ...
      <div className="flex-1 flex min-h-0 overflow-hidden relative">
        {/* Main Chart Area */}
        <div className="flex-1 relative p-4 sm:p-6 flex flex-col justify-end border-r border-[var(--border-subtle)] overflow-hidden">
          {/* Grid Lines */}
        ...
        {/* Order Book Panel */}
        <div className="w-28 sm:w-48 lg:w-64 bg-[var(--bg-base)] hidden md:flex flex-col overflow-hidden shrink-0">
          <div className="flex text-[8px] sm:text-[9px] font-semibold text-[var(--text-muted)] px-2 sm:px-4 py-2 border-b border-[var(--border-subtle)] uppercase tracking-widest shrink-0">
          ...
          <div className="flex-1 p-2 flex flex-col gap-[2px] overflow-hidden justify-center">
            ...
            {/* Spread */}
            <div className="my-1 sm:my-2 py-1 flex items-center justify-center border-y border-[var(--border-subtle)] shrink-0">
              <span className="text-sm sm:text-lg font-mono font-bold text-[var(--text-main)]">64,230.50</span>
            </div>
      ...
      <div className="absolute bottom-6 left-6 w-72 rounded-3xl bg-[var(--bg-base)]/90 border border-[var(--border-base)] shadow-xl backdrop-blur-md overflow-hidden hidden lg:block z-20">
        <div className="px-4 py-2 border-b border-[var(--border-subtle)] flex items-center justify-between">
          <span className="text-xs font-semibold text-[var(--text-main)]">Whale Pulse</span>
        ...
        <div className="p-3">
          <div className="flex gap-2 items-start">
            ...
            <div className="flex flex-col gap-0.5">
              <span className="text-xs font-bold text-[var(--text-main)]">Massive Transfer</span>
              <p className="text-[10px] text-[var(--text-muted)] leading-snug">12,500 BTC moving to spot wallets.</p>
            </div>
          </div>
        </div>
      </div>
  ...
  return (
    ...
    >
      <div className="mb-6 shrink-0">
        <h3 className="text-xs sm:text-sm text-[var(--text-muted)] font-medium mb-1">Total Balance</h3>
        <div className="text-2xl sm:text-4xl font-bold text-[var(--text-main)] font-mono">$124,592.80</div>
        <div className="text-[10px] sm:text-xs text-emerald-400 mt-1 flex items-center gap-1">
          <Activity size={12} /> +$4,230 (3.5%) Today
        </div>
      </div>
      
      <div className="flex-1 bg-[var(--bg-base)] border border-[var(--border-subtle)] rounded-3xl p-3 sm:p-4 flex flex-col gap-2 sm:gap-4 overflow-y-auto">
        {[
          ...
          <div key={asset.symbol} className="flex items-center justify-between p-2 sm:p-3 rounded-2xl bg-white/[0.02] hover:bg-white/[0.05] transition-colors border border-[var(--border-subtle)] shrink-0">
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <div>
                ...
                <div className="text-[9px] sm:text-[10px] text-[var(--text-muted)]">{asset.name}</div>
              </div>
            </div>
            <div className="text-right">
              <div className="font-mono font-bold text-xs sm:text-sm">{asset.value}</div>
              <div className="font-mono text-[9px] sm:text-[10px] text-[var(--text-muted)]">{asset.amount} {asset.symbol}</div>
            </div>
          </div>
        ))}
      </div>
...
function SwapTab() {
  return (
    ...
    >
      <div className="w-full max-w-sm bg-[var(--bg-base)] border border-[var(--border-subtle)] rounded-2xl p-4 sm:p-5 shadow-2xl relative">
        <h3 className="text-xs sm:text-sm font-bold text-[var(--text-main)] mb-4">Swap Assets</h3>
        
        <div className="bg-white/[0.02] border border-[var(--border-subtle)] rounded-3xl p-3 sm:p-4 mb-2">
          <div className="text-[9px] sm:text-[10px] text-[var(--text-muted)] mb-1">You pay</div>
          <div className="flex items-center justify-between">
            <span className="text-xl sm:text-2xl font-mono text-[var(--text-main)]">1.0</span>
            <div className="px-2 py-1 sm:px-3 sm:py-1 bg-white/10 rounded-full text-[10px] sm:text-xs font-bold">ETH</div>
          </div>
          <div className="text-[9px] sm:text-[10px] text-[var(--text-muted)] mt-1">$3,100.00</div>
        </div>

        <div className="absolute left-1/2 -translate-x-1/2 top-1/2 -translate-y-[15%] w-6 h-6 sm:w-8 sm:h-8 bg-[#111] border border-[var(--border-base)] rounded-full flex items-center justify-center z-10 shadow-lg">
          <ArrowDownUp size={12} className="text-[var(--text-muted)]" />
        </div>

        <div className="bg-white/[0.02] border border-[var(--border-subtle)] rounded-3xl p-3 sm:p-4 mb-4 sm:mb-6">
          <div className="text-[9px] sm:text-[10px] text-[var(--text-muted)] mb-1">You receive</div>
          <div className="flex items-center justify-between">
            <span className="text-xl sm:text-2xl font-mono text-[var(--text-main)]">3,095.50</span>
          ...
          <div className="text-[9px] sm:text-[10px] text-[var(--text-muted)] mt-1">$3,095.50</div>
        </div>

        <button className="w-full py-2 sm:py-3 bg-blue-600 hover:bg-blue-500 text-[var(--text-main)] font-bold rounded-3xl text-[10px] sm:text-sm transition-colors">
          Confirm Swap
        </button>
      </div>
    </motion.div>
...
function AiTab() {
  return (
    <motion.div 
      ...
      animate={{ opacity: 1, filter: "blur(0px)" }}
      exit={{ opacity: 0, filter: "blur(4px)" }}
      className="absolute inset-0 flex flex-col bg-[var(--bg-base)] overflow-hidden"
    >
      <div className="p-3 sm:p-4 border-b border-[var(--border-subtle)] flex items-center gap-3 bg-[var(--bg-base)] shrink-0">
      ...
      <div className="flex-1 p-3 sm:p-4 flex flex-col gap-4 overflow-y-auto">
        <div className="flex gap-2 sm:gap-3 justify-end">
          <div className="bg-blue-600 text-[var(--text-main)] text-[10px] sm:text-xs p-2 sm:p-3 rounded-2xl rounded-tr-sm max-w-[80%]">
        ...
        <div className="flex gap-2 sm:gap-3 items-start">
          <div className="w-5 h-5 sm:w-6 sm:h-6 rounded-full bg-white/10 flex items-center justify-center shrink-0 mt-1">
            <Bot size={10} className="text-[var(--text-main)]" />
          </div>
          <div className="bg-white/5 border border-[var(--border-base)] text-slate-200 text-[10px] sm:text-xs p-2 sm:p-3 rounded-2xl rounded-tl-sm max-w-[85%] leading-relaxed">
            Solana (SOL) is showing strong bullish divergence on the 4H timeframe. On-chain volume has increased by 14% in the last 24h. 
            <br/><br/>
            Key resistance: <b className="text-[var(--text-main)]"> $145</b><br/>
            Support: <b className="text-[var(--text-main)]"> $138</b>
            <br/><br/>
