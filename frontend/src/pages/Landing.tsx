      ...
      mockup: (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {[
            ...
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", borderRadius: 12, background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.05)" }}>
              {c.image_url ? (
                <img src={c.image_url} alt={c.sym} style={{ width: 32, height: 32, borderRadius: "50%", filter: "grayscale(30%)" }} />
              ) : (
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ height: 3, width: 50, borderRadius: 2, background: "rgba(255,255,255,0.1)", marginTop: 4, overflow: "hidden" }}>
                  <motion.div 
                    initial={{ width: 0 }} 
              ...
              <div style={{ textAlign: "right" }}>
                ...
                <div style={{ fontSize: 11, fontWeight: 700, color: c.up ? "rgba(255,255,255,0.6)" : "rgba(255,255,255,0.3)", fontFamily: "monospace" }}>{c.change}</div>
              </div>
            </div>
          ))}
        </div>
      ...
      mockup: (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={{ padding: "14px 16px", borderRadius: 12, background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)" }}>
            <div style={{ fontSize: 9, color: "rgba(255,255,255,0.7)", fontWeight: 800, letterSpacing: ".15em", marginBottom: 8 }}>[ NEKO AI ]</div>
            <div style={{ fontSize: 13, color: "rgba(255,255,255,0.5)", lineHeight: 1.6 }} dangerouslySetInnerHTML={{ __html: t('landing.feat2.mockup').replace('+18.4%', '<span style="color: #fff; font-weight: 700">+18.4%</span>').replace('15%', '<span style="color: #fff; font-weight: 700">15%</span>').replace('%18.4', '<span style="color: #fff; font-weight: 700">%18.4</span>').replace('%15', '<span style="color: #fff; font-weight: 700">%15</span>') }} />
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
            {[
              { l: t('landing.feat2.m_score'), v: "87/100", c: "#fff" },
              { l: t('landing.feat2.m_risk'), v: t('landing.feat2.m_risk_val'), c: "rgba(255,255,255,0.8)" },
              { l: t('landing.feat2.m_corr'), v: "0.72", c: "rgba(255,255,255,0.8)" },
              { l: t('landing.feat2.m_sharpe'), v: "1.84", c: "#fff" },
            ].map((s, i) => (
              <div key={i} style={{ padding: "12px 14px", borderRadius: 10, background: "rgba(255,255,255,0.01)", border: "1px solid rgba(255,255,255,0.04)" }}>
                <div style={{ fontSize: 9, color: "rgba(255,255,255,0.4)", marginBottom: 4 }}>{s.l}</div>
      ...
      mockup: (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
            <span style={{ fontSize: 10, color: "rgba(255,255,255,0.4)", fontWeight: 700, letterSpacing: ".1em", textTransform: "uppercase" }}>{t('landing.feat3.m_title')}</span>
            <span style={{ fontSize: 10, color: "rgba(255,255,255,0.8)", fontWeight: 700, background: "rgba(255,255,255,0.05)", padding: "2px 10px", borderRadius: 100, border: "1px solid rgba(255,255,255,0.1)" }}>FY 2024</span>
          </div>
          {[
            ...
            <div key={i} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderRadius: 10, background: "rgba(255,255,255,0.01)", border: "1px solid rgba(255,255,255,0.04)" }}>
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 12, fontWeight: 700, color: "#fff" }}>{t.asset} <span style={{ color: "rgba(255,255,255,0.4)", fontSize: 11 }}>{t.amount}</span></div>
                <div style={{ fontSize: 11, fontFamily: "monospace", color: "rgba(255,255,255,0.3)" }}>{t.value}</div>
              </div>
              {t.pnl && <span style={{ fontSize: 12, fontWeight: 800, color: "#34d399", fontFamily: "monospace" }}>{t.pnl}</span>}
            </div>
          ))}
          <div style={{ padding: "12px 16px", borderRadius: 12, background: "rgba(52,211,153,0.05)", border: "1px solid rgba(52,211,153,0.1)", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <span style={{ fontSize: 12, color: "rgba(255,255,255,0.5)" }}>{t('landing.feat3.m_total')}</span>
            <span style={{ fontSize: 20, fontWeight: 900, color: "#34d399", fontFamily: "monospace" }}>+$4,340</span>
          </div>
        </div>
      ),
    },
    ...
    <div className="mesh-hero" style={{ background: T.bg, color: T.textPrimary, fontFamily: "Inter, sans-serif", overflowX: "clip" }}>
      ...
      <section style={{ padding: "0 clamp(20px,5vw,80px)", maxWidth: 1200, margin: "0 auto 160px" }}>
        <div style={{ textAlign: "center", marginBottom: 80 }}>
          <Reveal>
            <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.15em", textTransform: "uppercase", color: "rgba(255,255,255,0.6)", marginBottom: 16 }}>{t('landing.features_header.badge')}</div>
            <h2 style={{ fontSize: "clamp(32px,5vw,56px)", fontWeight: 900, letterSpacing: "-0.04em", margin: 0, lineHeight: 1.1 }}>
              <span style={{ color: "#fff" }}>{t('landing.features_header.title')}</span><br />
        ...
        <div ref={featuresRef} style={{ position: "relative", userSelect: "none", WebkitUserSelect: "none" }}>
          {features.map((f, i) => {
            ...
            return (
              ...
              >
                {/* Subtle mono bg glow */}
                <div style={{ position: "absolute", top: "-40%", left: isRight ? "-10%" : "auto", right: isRight ? "auto" : "-10%", width: "60%", height: "180%", background: `radial-gradient(circle, rgba(255,255,255,0.03) 0%, transparent 60%)`, filter: "blur(80px)", pointerEvents: "none", zIndex: 0 }} />

                {/* Text */}
                <div style={{ position: "relative", zIndex: 1, order: isRight ? 1 : 2 }}>
                  <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "7px 18px", borderRadius: 100, background: `rgba(255,255,255,0.03)`, border: `1px solid rgba(255,255,255,0.08)`, color: "#fff", fontSize: 12, fontWeight: 800, letterSpacing: ".12em", marginBottom: 28, boxShadow: "none" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    {f.points.map((pt, j) => (
                      <div key={j} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                        ...
                        <span style={{ fontSize: 14, color: "rgba(255,255,255,0.6)" }}>{pt}</span>
                      </div>
                    ))}
                  </div>
                </div>
