...
export default function Market({ isWatched, toggleWatchlist }: any) {
  ...
  function TH({ label, sortKey, align = "right" }: { label: string; sortKey: string; align?: "left" | "right" }) {
    ...
    return (
      <div
        ...
        className={`flex items-center gap-1 cursor-pointer select-none text-[11px] font-bold uppercase tracking-widest transition-colors ${active ? "text-white" : "text-gray-500 hover:text-gray-300"}`}
  ...
  const Pagination = () => totalPages <= 1 ? null : (
    <div className="flex items-center gap-2">
      ...
      {getPages().map((p, i) =>
        p === "..." ? (
          <span key={`e-${i}`} className="w-7 text-center text-gray-500 text-xs">…</span>
        ) : (
          <button
            key={p}
    ...
    <div className="relative min-h-screen bg-[#0a0b0d] text-white pt-24 pb-32 px-6 lg:px-12 overflow-x-hidden font-sans">
      ...
      <FadeIn delay={0.2}>
        <div className="bg-[#16181c]/80 backdrop-blur-xl border border-[#273951]/50 shadow-2xl rounded-[32px] overflow-hidden">
          ...
          {isLoading && (
            <div className="p-2">
              {Array.from({ length: 15 }).map((_, i) => (
                <div key={i} className="h-14 mx-5 my-1 rounded-xl bg-white/[0.02] animate-pulse" />
              ))}
            </div>
          )}
          ...
          {/* Error */}
          {isError && (
            <div className="m-5 p-4 rounded-xl bg-red-500/10 border border-red-500/20 text-red-500 text-sm font-medium">
